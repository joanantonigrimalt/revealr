...
<body style="margin:0;padding:0;background:#faf9f7;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;color:#1a1814;">
  <div style="max-width:640px;margin:0 auto;padding:40px 24px;">
    ...
    ${result.missingInfo ? `<!-- Missing Info Banner -->
    <div style="background:#fffbeb;border:1px solid #fcd34d;border-radius:10px;padding:14px 18px;margin-bottom:20px;">
      <span style="font-weight:600;color:#92400e;">Note:</span>
      <span style="color:#78350f;font-size:14px;"> ${result.missingInfo}</span>
    </div>` : ''}
    ...
    ${counts.critical > 0 ? `<!-- Action Required Banner -->
    <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:16px 18px;margin-bottom:24px;">
      <div style="font-weight:700;color:#991b1b;font-size:15px;margin-bottom:10px;">⚠️ Action Required Before Signing</div>
      <div style="font-size:13px;color:#7f1d1d;margin-bottom:10px;">Your contract has ${counts.critical} critical issue${counts.critical > 1 ? 's' : ''} that should be addressed before you sign. The ${Math.min(counts.critical, 3)} most important:</div>
      ${flagsByType('critical').slice(0, 3).map((f) => `
      <div style="margin-top:10px;padding-top:10px;border-top:1px solid #fecaca;">
        <div style="font-weight:600;color:#991b1b;font-size:13px;">${f.title}</div>
        <div style="color:#7f1d1d;font-size:12px;margin-top:3px;">${f.section} · ${f.action}</div>
      </div>`).join('')}
    </div>` : ''}
    ...
    <div style="border-top:1px solid #e8e4df;padding-top:24px;color:#9c9590;font-size:12px;line-height:1.6;">
      <p style="margin:0 0 8px;">Your document has been permanently deleted from our servers.</p>
